...
function hackeryou_scripts() {
  ...
  wp_enqueue_script(
  	'jquery',
  	"http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js",
  	false, //dependencies
  	null, //version number
  	true //load in footer
  );
  ...
  wp_enqueue_script(
  	'slick',
  	"http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdn.jsdelivr.net/jquery.slick/1.6.0/slick.min.js",
  	false,
  	null,
  	true
  );
  ...
  wp_enqueue_script(
  	'gsap',
  	"http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdnjs.cloudflare.com/ajax/libs/gsap/1.19.1/TweenMax.min.js",
  	false,
  	null,
  	true
  );
  wp_enqueue_script(
    'mobile-detect', //handle
    get_template_directory_uri() . '/lib/mobile-detect/mobile-detect.js', //source
    false, //dependencies
    null, // version number
    true //load in footer
  );
